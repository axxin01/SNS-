...
################# prepare data ####################
data = read_excel("/Users/axin/Library/Containers/com.microsoft.Excel/Data/Downloads/data sns w&c.xlsx",sheet = "R_testing_warmth") # change here with "R_test_warmth" , "R_train_competence" and "R_test_competence"
...
results_path = "/Users/axin/Desktop"
...
table_bic <- xtable(BIC, digits = 4, caption = "BIC for different cost functions")
results_path = "/Users/axin/Desktop"
html_output <- print(table_bic, type = "html", comment = FALSE, caption.placement = "top", printl.results = FALSE)
footnote <- "<p> Notes on missing values: unable to find a global maximum for at least one of the cost models</p>"
con <- file(file.path(results_path, "table_bic.html"), open = "wt")
cat(html_output, footnote, sep = "\n", file = con)
close(con)
...
save.image("/Users/axin/Desktop") # or save to a specific location
